-- I was having trouble getting sourcemap.json to play nice with roselect, so I've included it as part of the project.
-- https://github.com/Rimuy/roselect
...
local function createStructuredSelector(selectors, selectorCreator)
	if type(selectors) ~= "table" then
		error(
			("createStructuredSelector expects first argument to be an object where each property is a selector, instead received a %s"):format(
				type(selectors)
			),
			2
		)
	...
	return selectorCreator(funcs, function(...)
